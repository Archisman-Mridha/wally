...
impl Default for WALOptions {
  fn default() -> Self {
    let dir = home_dir().unwrap_or("/".into()).join(".wally");
...
mod tests {
  ...
  fn it_works() -> Result<()> {
    ...
    let entries = vec![("HELLO WORLD", false), ("GOOD BYE", true), ("WE'RE SO BACK", false),];
    ...
    wal.checkpoint("WE'RE DONE".as_bytes().to_vec())?;
